...
c      dr=(3*m1**2 + 6*m1*m2 + 2*m2**2)/(arr*m)                         ! now calculated in mass2ddstg.f
      er=ecc*(1+dr)
c      dth=(3.5d0*m1**2 + 6*m1*m2 + 2*m2**2)/(arr*m)                    ! now calculated in mass2ddstg.f
      eth=ecc*(1+dth)
...
c  --> Inversion of timing model by iteration: end of loop
      ...
      ddSha_domega = -2.d0*bare_m2/brace * si * (-(cu - ecc)*cw + sqr1me2*su*sw) ! not used corretion to the derivative
